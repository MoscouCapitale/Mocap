...
export const handleErrorStatus = ({ status, email }: handleErrorStatusProps): FormResponse => {
  const renderObj: FormType = {
    type: "default",
    additional_data: {
      email: email,
    },
  ...
  switch (status) {
    ...
    case 422: // otp_disabled AuthApiError: Signups not allowed for otp : You cannot signin with email not saved
      ...
      renderObj.error!.message = "Cette adresse email n'est pas enregistrée, veuillez vous inscrire.";
      break;
    case 429:
      renderObj.error!.message = "Merci d'attendre quelques secondes avant de réessayer.";
      break;
    case 500:
    default:
