...
export default function Highlight({ content, size }: HighlightProps) {
  const isEmbed = useMemo(() => (content.link && content.is_embed) && getEmbedTargetFromLink(content.link), [
    content.link,
  ]);
  ...
  const renderMedia = useMemo(() => {
    if (isEmbed) {
      return <MediaEmbed link={content.link ?? ""} config={size ? { width: size.width, height: size.height } : {}} />;
    }
    ...
    if (content.media) {
      if (content.media.extension?.includes("video")) {
        ...
        return (
          <img
            className={cn(
              "absolute group-hover/main:brightness-75 w-full h-full rounded-[20px]",
  ...
  return (
    <div
      data-hover-card-embed={isEmbed}
      data-hover-card={!isEmbed}
      className={cn("group/main w-full h-full overflow-y-auto rounded-[20px] overflow-hidden")}
    >
      <div
        className={cn(
