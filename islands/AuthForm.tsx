...
export default function AuthForm({ data: { type, additional_data, error } }: { data: FormType }) {
  ...
  const handleInputChange = useCallback(
    (key: AuthFormFields, value: FormFieldValue) => {
      switch (key) {
        case "email":
          setEmail(value);
          break;
  ...
  return (
    <>
      <div class="w-full h-screen inline-flex justify-center items-center">
        <form class={cn("w-10/12 max-w-xs flex justify-center items-center flex-col gap-10 relative")} method="POST" onSubmit={() => setIsLoading(true)}>
          {(type === "default" || type === "signin") && (
            <>
              <input type="hidden" name="authtype" value="signin" />
              <Input field={formsInputs.email} onChange={(v) => handleInputChange("email", v)} />
              {/* FIXME: on signin, password is validated */}
          ...
          {type === "signup" && (
            <>
              ...
              <Button type={email && password && confirmpassword ? "submit" : "button"} disabled={!(email && password && confirmpassword)}>
                Demander des accès
              </Button>
            </>
          )}
