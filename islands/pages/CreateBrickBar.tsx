...
export default function CreateBrickBar({ brickType, brickData, returnBrick }: CreateBrickBarProps) {
  ...
  const saveBrick = useCallback(
    (withCanvaInsert?: boolean) => {
      if (!brick) return;
      const brickState = ({ ...brick, type: brickType } as availBricks);  
      if (withCanvaInsert && MCNodes.find((n) => n.type === BricksType.HeroSection) && brickState.type === BricksType.HeroSection) {
        toast({
      ...
      ky.put("/api/brick", {
        ...
        .then((res) => {
          if (res) {
            ...
            toast({
              title: "Brick saved",
              description: `The brick ${brickState.name} has been saved.`,
            });
