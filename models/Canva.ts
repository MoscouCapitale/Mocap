...
export type { DBMNode, MNode };

export const getAvailableSizes = (node: Partial<MNode> & { type: keyof typeof BricksType }): MNodeSize[] => {
  const { type } = node;
  const embed = node.content && ("is_embed" in node.content && node.content.is_embed) && "link" in node.content
    ? getEmbedTargetFromLink(node.content.link as string ?? "")
    : null;
